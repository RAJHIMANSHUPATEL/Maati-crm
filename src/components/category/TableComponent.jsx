...
const TableComponent = ({ categories, setCategories }) => {
  ...
  const handleStatusToggle = async (_id, status) => {
    try {
      ...
      const res = await categoryAPI.updateCategoryStatus(data, authToken);

      if (res.success) {
        setCategories((prevCategories) =>
          prevCategories.map((category) =>
            category._id === _id ? { ...category, status: newStatus } : category
          )
        );
        toast.success(
                ...
                <CAvatar
                  ...
                  src={
                    item?.cover
                      ...
                      : `https://ui-avatars.com/api/?name=ProdCategoryuct&background=ddd&color=555`
                  }
