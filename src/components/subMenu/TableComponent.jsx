...
const TableComponent = ({ subMenus, setSubMenus }) => {
  ...
  const handleStatusToggle = async (_id, status) => {
    try {
      ...
      const res = await subMenuAPI.updateSubMenuStatus(data, authToken);

      if (res.success) {
        setSubMenus((prevSubCatogeries) =>
          prevSubCatogeries.map((subMenu) =>
            subMenu._id === _id
              ? { ...subMenu, status: newStatus }
              : subMenu
          )
        );
        // Show success toast
        toast.success(
          `Sub-menu successfully ${
            newStatus === "active" ? "activated" : "deactivated"
          }`
        );
      }
    } catch (error) {
      console.error("Error Updating Sub Menu Status ", error);
      toast.error("Failed to update sub-menu status");
    }
  };
  ...
  const filteredSubMenus =
    filterStatus === "all"
      ? subMenus
      : subMenus.filter(
          (subMenu) => subMenu.status === filterStatus
        );
